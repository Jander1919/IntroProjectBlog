...
echo_pretty "Starting kiteweb provisioner..."
...
if [ ! -d "/home/vagrant/.rvm/rubies/default/" ]; then
    ...
    echo_pretty "Installing Ruby 2.3.0..."
    # Install Ruby
    su -l -c 'rvm install 2.3.0' vagrant
    su -l -c 'rvm --default use 2.3.0' vagrant
...
if [ ! -f "/usr/bin/psql" ]; then
    ...
    sudo -u postgres createuser -s kite_development
    sudo -u postgres psql -c "ALTER USER kite_development WITH PASSWORD 'kite_development'"
else
...
if [ ! -f "/usr/share/elasticsearch/bin/elasticsearch" ]; then
    ...
    pushd /tmp
    wget --quiet https://download.elasticsearch.org/elasticsearch/elasticsearch/elasticsearch-2.2.1.deb
    echo_pretty "Configuring and installing ElasticSearch..."
    dpkg -i elasticsearch-2.2.1.deb
    rm -rf elasticsearch*
else
